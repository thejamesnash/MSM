var extraStuff = function(){
    ...
    var jsonCallback = function(data){
        ...
        if (userData.gas_unit_promo > 0) {
            document.querySelector('.gasUnitRate').textContent = userData.gas_unit_promo;
            document.querySelector('.elecUnitRate').textContent = userData.electric_unit_promo;
        }
        else {
            document.querySelector('.gasUnitRate').textContent = userData.gas_unit_std;
            document.querySelector('.elecUnitRate').textContent = userData.electric_unit_std;
        }

        document.querySelector('.gasStdCharge').textContent = userData.gas_charge_std;
        document.querySelector('.elecStdCharge').textContent = userData.electric_charge_std;

        // Get current tariff
        // var currentTariffCost = getAnnualPrice(userGasStandingCharge,userGasTariff,userElecStandingCharge,userElecTariff);
    }
    AJAX_JSON_Req('assets/energy-data-test.json',jsonCallback);
}